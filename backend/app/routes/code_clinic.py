...
from app.models import CodeClinicPost, CodeClinicComment
from app.utils.decorators import require_owner, get_current_user

bp = Blueprint("code_clinic", __name__, url_prefix="/api/code-clinic")

MEDIA_TYPES = {"video", "image", "none"}


def _post_owner_id(post_id):
    post = db.session.get(CodeClinicPost, post_id)
    return post.user_id if post else None


@bp.get("")
def list_posts():
    posts = CodeClinicPost.query.order_by(CodeClinicPost.created_at.desc()).all()
...
def get_post(post_id):
    ...
    return jsonify({"success": True, "post": post.to_dict(detailed=True)})


@bp.patch("/<int:post_id>")
@require_owner(_post_owner_id)
...
def add_comment(post_id):
    ...
    comment = CodeClinicComment(code_clinic_post_id=post.id, user_id=current_user.id, body=body)
